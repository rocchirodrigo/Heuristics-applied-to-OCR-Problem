%% Coordenação entre 2 Relés de Sobrecorrente, Curva Inversa IEC
            ...
            for R = 1:N_Reles
                for count = 1:count_MAX
                    if((tabu_short_mem(count,R) == 0)&& rest_carga(count,R) == 0)
                        ...
                        Kij_geral = zeros(28,N_Reles);
                        
                        Kij_geral(1,2) = B1(1,2); % prim. max
                        Kij_geral(1,1) = -B1(2,2); % ret. max
                        Kij_geral(2,2) = B2(1,2); % prim. min
                        Kij_geral(2,1) = -B2(2,2); % ret. min
                        
                        
                        for k = 1:2
                            % Tmin
                            Kij_geral(2+k,k) = -B1(1,k);
                            Kij_geral(4+k,k) = -B2(1,k);
                            % Tmax
                            Kij_geral(6+k,k) = B1(1,k);
                            Kij_geral(8+k,k) = B2(1,k);
                        end
...
for i = 1:N_Reles;
    T_pri1(i,1) = x(i,1) * best_B1(1,i);
    T_ret1(i,1) = x(q(i),1) * best_B1(2,i);
    T_pri2(i,1) = x(i,1) * best_B2(1,i);
    T_ret2(i,1) = x(q(i),1) * best_B2(2,i);
end
